...
class TestCommand:
    ...
    async def test_get_command_list_with_active_status(self, client: AsyncClient, test_category):
        """测试按启用状态过滤指令"""
        # 创建启用和禁用的指令
        await client.post(
            COMMAND_BASE_URL,
    ...
    async def test_get_command_list_pagination(self, client: AsyncClient, test_category):
        """测试指令列表分页"""
        # 创建多个指令
        for i in range(5):
            await client.post(
    ...
    async def test_get_command_list_order_by_priority(self, client: AsyncClient, test_category):
        """测试指令列表按优先级排序"""
        # 创建不同优先级的指令
    ...
    async def test_update_command_duplicate_name(self, client: AsyncClient, test_category):
        """测试更新指令为重复名称"""
        # 创建两个指令
    ...
    async def test_update_command_duplicate_code(self, client: AsyncClient, test_category):
        """测试更新指令为重复代码"""
        # 创建两个指令
        cmd1 = await client.post(
            COMMAND_BASE_URL,
