...
SENSITIVE_BASE_URL = "/api/v1/sensitive"
...
class TestSensitiveCategory:
    ...
    async def test_get_category_list(self, client: AsyncClient, test_category):
        """测试获取分类列表"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_get_category_list_with_search(self, client: AsyncClient, test_category):
        """测试搜索分类列表"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category", params={"nameOrDesc": "测试"})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
        assert data["data"]["total"] >= 1
    ...
    async def test_get_category_list_pagination(self, client: AsyncClient):
        ...
        # 测试分页
        response = await client.get(f"{SENSITIVE_BASE_URL}/category", params={"current": 1, "size": 3})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_get_category_detail(self, client: AsyncClient, test_category):
        """测试获取分类详情"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category/{test_category['id']}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_get_category_detail_not_found(self, client: AsyncClient):
        """测试获取不存在的分类"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category/99999")
    ...
    async def test_update_category_success(self, client: AsyncClient, test_category):
        ...
        response = await client.put(
            f"{SENSITIVE_BASE_URL}/category/{test_category['id']}",
    ...
    async def test_update_category_duplicate(self, client: AsyncClient):
        ...
        # 尝试将分类B更新为分类A的名称
        response = await client.put(
            f"{SENSITIVE_BASE_URL}/category/{cat2_data['id']}",
    ...
    async def test_delete_category_success(self, client: AsyncClient):
        ...
        # 删除分类
        response = await client.delete(f"{SENSITIVE_BASE_URL}/category/{category_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_delete_category_not_found(self, client: AsyncClient):
        """测试删除不存在的分类"""
        response = await client.delete(f"{SENSITIVE_BASE_URL}/category/99999")
        assert response.status_code == 404
        data = response.json()
...
class TestSensitiveWord:
    ...
    async def test_get_word_list(self, client: AsyncClient, test_word):
        """测试获取敏感词列表"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/word")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
        assert "items" in data["data"]
    ...
    async def test_get_word_list_with_filters(self, client: AsyncClient, test_word):
        """测试过滤敏感词列表"""
        # 按词搜索
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"word": "测试"})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"

        # 按分类过滤
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"categoryId": test_word["categoryId"]})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
        assert data["data"]["total"] >= 1
        ...
        # 按级别过滤
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"level": 1})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_get_word_list_pagination(self, client: AsyncClient, test_category):
        ...
        # 测试分页
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"current": 1, "size": 3})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
        assert len(data["data"]["items"]) <= 3
    ...
    async def test_get_word_detail(self, client: AsyncClient, test_word):
        """测试获取敏感词详情"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/word/{test_word['id']}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
    ...
    async def test_get_word_detail_not_found(self, client: AsyncClient):
        """测试获取不存在的敏感词"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/word/99999")
        assert response.status_code == 404
        data = response.json()
        assert "不存在" in data["detail"]
    ...
    async def test_update_word_success(self, client: AsyncClient, test_word):
        """测试更新敏感词成功"""
        response = await client.put(
            f"{SENSITIVE_BASE_URL}/word/{test_word['id']}",
            json={
                "word": "更新后的敏感词",
                "categoryId": test_word["categoryId"],
                "level": 3,
    ...
    async def test_update_word_not_found(self, client: AsyncClient, test_category):
        """测试更新不存在的敏感词"""
        response = await client.put(
            f"{SENSITIVE_BASE_URL}/word/99999",
            json={
                "word": "不存在的词",
                "categoryId": test_category["id"],
                "level": 1,
    ...
    async def test_update_word_duplicate(self, client: AsyncClient, test_category):
        ...
        # 尝试将敏感词B更新为敏感词A
        response = await client.put(
            f"{SENSITIVE_BASE_URL}/word/{word2_data['id']}",
            json={
                "word": "敏感词A",
                "categoryId": test_category["id"],
                "level": 1,
    ...
    async def test_delete_word_success(self, client: AsyncClient, test_category):
        ...
        # 删除敏感词
        response = await client.delete(f"{SENSITIVE_BASE_URL}/word/{word_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
        assert "成功" in data["msg"]
    ...
    async def test_delete_word_not_found(self, client: AsyncClient):
        """测试删除不存在的敏感词"""
        response = await client.delete(f"{SENSITIVE_BASE_URL}/word/99999")
        assert response.status_code == 404
        data = response.json()
        assert "不存在" in data["detail"]
...
class TestEdgeCases:
    ...
    async def test_category_name_max_length(self, client: AsyncClient):
        """测试分类名称长度限制"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category", params={"nameOrDesc": "a" * 11})
        # 应该会因为验证失败返回422
        assert response.status_code == 422

    async def test_word_max_length(self, client: AsyncClient):
        """测试敏感词长度限制"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"word": "a" * 51})
        # 应该会因为验证失败返回422
        assert response.status_code == 422
    ...
    async def test_level_validation(self, client: AsyncClient):
        """测试级别验证"""
        # 测试超出范围的级别
        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"level": 4})
        assert response.status_code == 422

        response = await client.get(f"{SENSITIVE_BASE_URL}/word", params={"level": 0})
        assert response.status_code == 422

    async def test_empty_list(self, client: AsyncClient):
        """测试空列表"""
        response = await client.get(f"{SENSITIVE_BASE_URL}/category", params={"nameOrDesc": "不存在的搜索词"})
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == "0000"
