...
class ChatCache:
    ...
    def add_message(self, chat_id: str, text: str, role: str):
        if chat_id not in self.cache:
            self.cache[chat_id] = []
        if role == "user":
            self.cache[chat_id].append(ChatMessage.from_user(text))
        else:
            self.cache[chat_id].append(ChatMessage.from_assistant(text))
