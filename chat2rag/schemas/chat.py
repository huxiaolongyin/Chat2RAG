...
class ChatQueryParams(BaseSchema):
    ...
    batch_or_stream: ProcessType = Field(
        ProcessType.BATCH, description="流式响应或者批式响应"
    )
    prompt_name: str = Field("默认", description="提示词名称选择")
    generator_model: str = Field(
        default=CONFIG.CHAT_V1_DEFAULT_MODELS["generator"], description="生成模型"
    )
...
class ChatRequest(BaseSchema):
    ...
    model: str = Field(
        default=CONFIG.MODEL, description="模型名称，非必填，使用默认配置模型"
    )
    ...
    collections: list = Field(
        default=[], description="知识库名称，非必填，支持多个，空值为不进行知识库检索"
    )
    score_threshold: float = Field(
    ...
    top_k: int = Field(
        default=CONFIG.TOP_K, ge=0, le=30, description=f"  : {CONFIG.TOP_K}"
    )
    batch_or_stream: ProcessType = Field(
    ...
    content: QueryContent = Field(
        ..., description="交互内容，必填，支持类型：text、image、video、audio"
    )
    chat_id: str | None = Field(
        None, description="会话标识，非必填，用于处理多轮聊天会话"
    )
    ...
    extra_params: dict = Field(
        default={}, description="额外参数，非必填，传入prompt提示词的参数"
    )
...
class StreamChunkV1(BaseSchema):
    ...
    content: str = Field(
        ..., description="响应消息内容", examples=["你好！有什么可以帮您的吗？"]
    )
    model: str = Field("None", description="调用模型名称", examples=["qwen-32b"])
    status: int = Field(
        ..., description="回复状态，0: 准备回复、1:回复开始、2:回复结束", examples=[0]
    )
...
class AudioContent(BaseSchema):
    text: str = Field(..., description="音频对应的文字内容")
...
class ContentSchema(BaseSchema):
    ...
    image: str = Field(
        "", description="图片内容或URL", examples=["image_url_or_base64"]
    )
    video: str = Field("", description="视频内容或URL", examples=["video_url"])
    audio: AudioContent | None = Field(None, description="音频数据")
...
class StreamChunkV2(BaseSchema):
    object: str = Field("message", description="数据类型", examples=["message"])
    input: dict = Field(
        {}, description="用户输入内容", examples=["你好，请帮我查询天气"]
    )
    content: ContentSchema = Field(..., description="响应内容，包含文本、图片、音频")
    model: str = Field("None", description="调用模型名称", examples=["qwen-32b"])
    status: int = Field(
        ..., description="回复状态，0: 准备回复、1:回复开始、2:回复结束", examples=[0]
    )
