...
class CommandCategoryBase(BaseSchema):
    ...
    description: str | None = Field(
        None, max_length=255, description="描述信息", examples=["测试描述"]
    )
...
class CommandCategoryUpdate(BaseSchema):
    name: str | None = Field(
        None, max_length=50, description="分类名称", examples=["测试"]
    )
    description: str | None = Field(
        None, max_length=255, description="描述信息", examples=["测试描述"]
    )
...
class CommandVariantBase(BaseSchema):
    text: str = Field(
        ..., max_length=255, description="指令文本", examples=["音量调整到{num}"]
    )
    pattern: str | None = Field(
        None, max_length=255, description="参数匹配模式", examples=["音量调整到{num}"]
    )
...
class CommandBase(BaseSchema):
    ...
    description: str | None = Field(
        None, max_length=255, description="描述信息", examples=["左转"]
    )
    ...
    examples: list[str] = Field(
        default_factory=list, description="示例说法列表，用于LLM few-shot识别"
    )


class CommandData(CommandBase, IDMixin, TimestampMixin):
    variants: list[CommandVariantData] = Field(
        default_factory=list, description="指令变体列表"
    )


class CommandCreate(CommandBase):
    variants: list[CommandVariantCreate] | None = Field(
        None, description="指令变体列表"
    )


class CommandUpdate(BaseSchema):
    name: str | None = Field(
        None, max_length=50, description="指令名称", examples=["向左转"]
    )
    code: str | None = Field(
        None, max_length=50, description="指令代码", examples=["TurnLeft"]
    )
