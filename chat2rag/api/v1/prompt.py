...
@router.get(
    "/list",
    response_model=BaseResponse[PromptPaginatedData],
    summary="获取提示词列表",
    deprecated=True,
)
@router.get(
    "", response_model=BaseResponse[PromptPaginatedData], summary="获取提示词列表"
)
...
@router.get(
    "/version",
    response_model=BaseResponse[PromptData],
    summary="设置指定提示词的活跃版本",
)
...
@router.get(
    "/{prompt_id}",
    response_model=BaseResponse[PromptDetailData],
    summary="获取提示词详情",
)
async def get_detail(prompt_id: int = Path(..., gt=0, description="提示词ID")):
    prompt = await prompt_service.get_detail(prompt_id)
...
async def create_prompt(prompt_in: PromptCreate):
    ...
    return BaseResponse.success(
        data=PromptData.model_validate(prompt), msg="提示词创建成功"
    )


@router.put(
    "/update/{prompt_id}",
    response_model=BaseResponse[PromptIdResponse],
    summary="更新提示词",
    deprecated=True,
)
@router.put(
    "/{prompt_id}", response_model=BaseResponse[PromptIdResponse], summary="更新提示词"
)
async def update_prompt(prompt_id: int, prompt_in: PromptUpdate):
    prompt = await prompt_service.update(prompt_id, prompt_in)
    return BaseResponse.success(
        data=PromptIdResponse(prompt_id=prompt.id), msg="提示词更新成功"
    )
