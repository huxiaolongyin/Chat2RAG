...
from chat2rag.database.models import Metric
from chat2rag.database.services.base_service import BaseService, Paginator
...
class MetricService(BaseService[Metric, MetricCreate, MetricUpdate]):
    """
    Metrics表的高级服务类，提供指标数据的综合分析功能
    """

    def __init__(self):
        super().__init__(Metric)

    def get_by_message_id(self, db: Session, message_id: str) -> Optional[Metric]:
        """通过消息ID获取指标记录"""
        return db.query(Metric).filter(Metric.message_id == message_id).first()

    def get_by_chat_id(
        self, db: Session, chat_id: str, page: int = 1, page_size: int = 20
    ) -> Paginator:
        """获取指定会话的所有指标记录（分页）"""
        filters = [Metric.chat_id == chat_id]
        order_by = [self.order_by_field("create_time")]

        return self.get_paginated(
            db=db, page=page, page_size=page_size, filters=filters, order_by=order_by
        )
    ...
    def get_latest_metrics(self, db: Session, limit: int = 10) -> List[Metric]:
        """
        获取最新的指标记录
        ...
        Returns:
            最新的指标记录列表
        """
        return db.query(Metric).order_by(desc(Metric.create_time)).limit(limit).all()
    ...
    def update_metric(
        ...
        Returns:
            更新后的指标记录或None
        """
        db_obj = self.get_by_message_id(db, message_id)
        if db_obj:
            return self.update(db, db_obj=db_obj, obj_in=metric_data)
        return None

    def create_metric(
        self, db: Session, metric_data: Union[MetricCreate, Dict[str, Any]]
    ) -> Metric:
        """
        创建新的指标记录
    ...
    def get_tool_usage_stats(
        ...
        result = db.execute(text(sql), {"start_time": start_time, "end_time": end_time})
    ...
    def get_error_stats(
        ...
        # 总请求数
        total_count = db.query(func.count(Metric.message_id)).filter(*filters).scalar()

        # 错误请求数（有错误信息的请求）
        error_filters = filters.copy()
        error_filters.append(Metric.error_message.isnot(None))
        error_count = (
            db.query(func.count(Metric.message_id)).filter(*error_filters).scalar()
        )
        ...
        # 获取常见错误类型及数量
        common_errors_query = (
            db.query(Metric.error_message, func.count(Metric.message_id).label("count"))
    ...
    def get_time_series_data(
        ...
        query = (
            db.query(time_expr, metric_expr)
            .filter(*filters)
            .group_by(time_expr)
            .order_by(time_expr)
        )
    ...
    def get_chat_completion_metrics(self, db: Session, chat_id: str) -> Dict[str, Any]:
        """
        获取特定会话的完整指标统计
        ...
        # 获取会话基本信息
        first_message = (
            db.query(Metric).filter(*filters).order_by(Metric.create_time).first()
        )
        last_message = (
            db.query(Metric).filter(*filters).order_by(desc(Metric.create_time)).first()
        )
        ...
        # 获取会话轮次
        rounds = db.query(func.count(Metric.message_id)).filter(*filters).scalar()
        ...
        # 获取平均响应时间
        avg_response_time = (
            db.query(func.avg(Metric.total_ms)).filter(*filters).scalar()
        )

        # 获取工具使用情况
        tools_used = []
        tools_query = (
            db.query(Metric.tools).filter(*filters).filter(Metric.tools.isnot(None))
        )
    ...
    def export_metrics_to_dict(
        ...
        metrics = (
            db.query(Metric).filter(*base_filters).order_by(Metric.create_time).all()
        )
    ...
    def get_dashboard_summary(self, db: Session, days: int = 7) -> Dict[str, Any]:
        """
        获取仪表盘概要数据
        ...
        total_requests = self.count(
            db, filters=[self.between_dates("create_time", start_time, end_time)]
        )

        # 获取今日请求数
        today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        today_requests = self.count(
            db, filters=[self.between_dates("create_time", today_start, end_time)]
        )

        # 获取平均响应时间
        avg_response_time = (
            db.query(func.avg(Metric.total_ms))
        ...
        # 获取模型使用分布
        model_usage = self.get_model_usage_stats(db, start_time, end_time)
