...
import type { RobotAction, RobotActionCreate } from "@/types/api";
...
function openEditDrawer(action: RobotAction) {
  isEdit.value = true;
...
async function handleSubmit() {
  ...
  if (!form.code.trim()) {
    Message.warning("请输入动作代码");
    return;
  }

  drawerLoading.value = true;
  try {
    const payload: RobotActionCreate = {
      name: form.name.trim(),
      code: form.code.trim(),
    ...
    if (isEdit.value && editId.value) {
      await actionApi.updateAction(editId.value, payload);
...
async function toggleActionStatus(action: RobotAction) {
  try {
    await actionApi.updateAction(action.id, { isActive: !action.isActive });
    Message.success(action.isActive ? "已禁用" : "已启用");
    loadActions();
  } catch (e) {
...
async function handleExport(format: "xlsx" | "csv") {
  ...
  try {
    const response = await actionApi.exportActions({
      format,
      actionIds: selectedRowKeys.value.length > 0 ? selectedRowKeys.value : undefined,
      isActive:
    ...
    link.href = url;
    link.download = format === "csv" ? "actions.csv" : "actions.xlsx";
    ...
    console.error("Failed to export actions:", e);
    Message.error("导出失败");
  } finally {
...
async function handleDownloadTemplate() {
  try {
    const response = await actionApi.downloadTemplate();
    const blob = new Blob([response.data], {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = "action_template.xlsx";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
...
async function handleImport(file: File) {
  ...
  try {
    const result = await actionApi.importActions(file);
    if (result.code === "0000") {
    ...
    console.error("Failed to import actions:", e);
    Message.error("导入失败");
  } finally {
      ...
      >
        <template #columns>
          ...
          <a-table-column title="操作" :width="160" align="center">
            <template #cell="{ record }">
              <a-space>
                <a-button type="text" size="small" @click="toggleActionStatus(record)">
                  <template #icon>
                    <Icon
                      :icon="
                        record.isActive
    ...
    >
      <a-form :model="form" layout="vertical">
        <a-form-item label="动作名称" required>
          <a-input v-model="form.name" placeholder="请输入动作名称" />
        </a-form-item>
        <a-form-item label="动作代码" required>
          <a-input v-model="form.code" placeholder="请输入动作代码，如 WaveHands" />
        </a-form-item>
